...
public class JogoService {
	...
	public void makeForm(int tipo, Jogo jogo, int orderBy) {
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Data de Anúncio: <input class=\"input--register\" type=\"text\" name=\"dataAnuncio\" value=\""+ jogo.getDataAnuncio().toString() + "\"></td>";
			umJogo += "\t\t\t<td>Data de lancamento: <input class=\"input--register\" type=\"text\" name=\"dataLancamento\" value=\""+ jogo.getDataLancamento().toString() + "\"></td>";
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Data de Anúncio: "+ jogo.getDataAnuncio().toString() + "</td>";
			umJogo += "\t\t\t<td>Data de lancamento: "+ jogo.getDataLancamento().toString() + "</td>";
			umJogo += "\t\t\t<td>&nbsp;</td>";
			umJogo += "\t\t</tr>";
			umJogo += "\t</table>";		
		} else {
		...
		list += "\n<tr><td colspan=\"6\" align=\"left\"><font size=\"+2\"><b>&nbsp;&nbsp;&nbsp;Relação de Jogos</b></font></td></tr>\n" +
        		...
        		"\t<td><a href=\"/jogo/list/" + FORM_ORDERBY_DESCRICAO + "\"><b>Nome</b></a></td>\n" +
		...
		} else if (orderBy == FORM_ORDERBY_DESCRICAO) {		jogos = jogoDAO.getOrderByNome();
	...
	public Object insert(Request request, Response response) {
		...
		LocalDateTime dataAnuncio = LocalDateTime.parse(request.queryParams("dataAnuncio"));
		LocalDate dataLancamento = LocalDate.parse(request.queryParams("dataLancamento"));
		
		String resp = "";
		
		Jogo jogo = new Jogo(-1, nome, preco, quantidade, dataAnuncio, dataLancamento);
	...
	public Object update(Request request, Response response) {
        ...
        if (jogo != null) {
        	...
        	jogo.setDataAnuncio(LocalDateTime.parse(request.queryParams("dataAnuncio")));
        	jogo.setDataLancamento(LocalDate.parse(request.queryParams("dataLancamento")));
        	jogoDAO.update(jogo);
        	response.status(200); // success
